...
$(function(){
	...
	//등록 버튼 클릭 시
	$("#s_insert").click(function(){
		var space3=document.writeform.passwd.value.replace(/\s{1}|　/gi, "");
		//var space3=document.writeform.nickname.value.replace(/^\s+|\s+$/g, "");
		...
		//비밀번호 확인 공백이거나 비밀번호와 일치 않으면 실행
			...
			if ( ($("#passwd").val().length > 8) ){
				...
				$("#passwordHelper").text("최소 3자리 이상 입력해 주세요.");
				//비밀번호에 포커스
				$("#passwd").focus();
				return false;
			}
...
$(function(){
	...
	$("#s_insert").click(function(){
		...
		if( space4 == ""){
			if( $("#nickname").val().length > 8 && space4 == ""){
				//오류 출력
				$("#nicknameHelper").text("닉네임을 2자리 ~ 8자리까지 입력해 주세요.");
				//닉네임에 포커스
				$("#nickname").focus();
				return false;	//flase로 리턴 하여 true일 때만 실행하게 설정
			}else if( (space4 == "") || document.writeform.nickname.value.match(nickname2) ){
				$("#nicknameHelper").text("공백을 넣을 수 없습니다.");
				//비밀번호에 포커스
				$("#nickname").focus();
				return false;
			}else{
