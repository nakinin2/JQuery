...
$(function(){
	...
	$("#s_insert").click(function(name){
		var name1=document.writeform.name.value.replace(/\s|　/gi, "");
		var num1=/^([a-z]|[ㄱ-힣]){0}[a-zA-Zㄱ-힣]{1,10}$/;
		//닉네임이 공백이거나 1자 미만일 경우 실행
		if( name1 == ""){
			if( $("#name").val().length > 8 && !name1){
				//오류 출력
...
$(function(){
	...
	$("#s_insert").click(function(){
		...
		//이름이 공백이나 숫자이거나 2자 미만이면 실행
			...
			if(  $("#id").val().length > 8 && !document.writeform.id.value.match(num2) ){
				$("#idHelper").text("최대 8자리를 넘을 수 없습니다.");
				//아이디에 포커스
				$("#id").focus();
				return false;
			}else if(  $("#id").val().length < 2 && !document.writeform.id.value.match(num2) ){
...
$(function(){
	...
	$("#s_insert").click(function(){
		...
		//비밀번호 확인 공백이거나 비밀번호와 일치 않으면 실행
			...
			if ( ($("#passwd").val().length > 5) && !document.writeform.passwd.value.match(passnum1) ){
				$("#passwordHelper").text("최대 5자리를 넘을 수 없습니다.");
				//아이디에 포커스
				$("#passwd").focus();
				return false;
			}else if ( ($("#passwd").val().length < 3) && !document.writeform.passwd.value.match(passnum1) ){
				$("#passwordHelper").text("최소 3자리 이상 입력하세요.");
				//아이디에 포커스
				$("#passwd").focus();
				return false;
			}
			...
			$("#passwd").focus();
			return false;
		}else if ( ($("#passwd").val().length > 5) && document.writeform.passwd.value.match(passnum1) ){
			$("#passwordHelper").text("최대 5자리를 넘을 수 없습니다.");
			//아이디에 포커스
			$("#passwd").focus();
			return false;
...
$(function(){
	...
	$("#nickname").focus();
	$("#s_insert").click(function(){
		var nickname1=document.writeform.nickname.value.replace(/\s|　/gi, "");
		var nickname2=/([!,@,#,$,%,^,&,*,?,_,~])|([!,@,#,$,%,^,&,*,?,_,~])/;
		//닉네임이 공백이거나 비밀번호 확인이 공백이면 실행
		if( nickname1 == ""){
			if( $("#nickname").val().length > 8 && !nickname1){
				//오류 출력
...
$(function(){
	...
	$("#s_insert").click(function(){
		...
		//var phone2 = /^01([0|1|6|7|8|9]?)-?([0-9]{3,4})-?([0-9]{4})$/;
		//전화번호가 공백이거나 
		if( $("#tel").val() == "" ){
  		//오류 출력
			...
			$("#telHelper").text("휴대폰 번호가 유효하지 않습니다.1");
			$("#tel").focus();
			return false; //flase로 리턴 하여 true일 때만 실행하게 설정
		}else{
			$("#telHelper").text("");
